...
(function () {
    ...
    function modifyPage(storedSettings) {
        ...
        const openInNewTab = storedSettings.extensionSettings.useNewTab ? "target=\"_blank\"" : "";
        const protocol = storedSettings.extensionSettings.useHttps ? "https://" : "http://";
        const jiraUrlToUse = protocol + storedSettings.extensionSettings.jiraUrl;
                ...
                for (let pr of pullRequests) {
                    if (unlinked(pr, jiraUrlToUse)) {
                        ...
                        if (prLinks && prLinks.length == 1) {
                            const originalPrLinkObj = prLinks[0];
                            ...
                            const newPrLinks = pr.getElementsByClassName('js-navigation-open');
                            if (newPrLinks == 2) {
